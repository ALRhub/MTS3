...
class Infer:
    ...
    def predict(self, obs: np.ndarray, act: np.ndarray, targets: np.ndarray, 
        ...
        dataset = TensorDataset(obs,  act, targets, obs_valid, task_valid)
        ...
        for batch_idx, (obs_batch, act_batch, targets_batch, obs_valid_batch, task_valid) in enumerate(loader):
            with torch.no_grad():
