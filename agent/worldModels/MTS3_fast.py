...
class MTS3(nn.Module):
    ...
    def forward(self, obs_seqs, action_seqs, obs_valid_seqs):
        ...
        task_prior_mean_init, task_prior_cov_init = self._intialize_mean_covar(obs_seqs.shape[0],
                                                                                scale=self.c.mts3.manager.initial_state_covar, learn=False)
        skill_prior_mean, skill_prior_cov = self._intialize_mean_covar(obs_seqs.shape[0], diagonal=True, learn=False)

        #### Parallel Encoding of the observation and action sequences
        if self.c.mts3.parallel:
            ...
            time_embedding_parallel = time_embedding[:, :obs_seqs.shape[1], :]  ## trim the time embedding to the length of the sequence
            # [x] made sure works with episodes < H
            beta_k_mean_parallel, beta_k_var_prallel = self._absObsEnc(torch.cat([obs_seqs, time_embedding_parallel], dim=-1))

            # [x] made sure works with episodes < H
            alpha_k_mean_parallel, alpha_k_var_parallel = self._absActEnc(torch.cat([action_seqs, time_embedding_parallel], dim=-1))
        ...
        for k in range(0,num_episodes):  ## first episode is considered too to predict (but usually ignored in evaluation)
            ...
            ### encode the observation (no time embedding) in parallel for the whole episode
            if self.c.mts3.parallel:
                obs_mean_parallel, obs_var_parallel = self._obsEnc(current_obs_seqs)
            for t in range(current_episode_len):  # [x] made sure works with episodes < H
                ### get current obs mean and var
                if self.c.mts3.parallel:
                    obs_mean = obs_mean_parallel[:, t, :]
                    obs_var = obs_var_parallel[:, t, :]
                else:
                    obs_mean, obs_var = self._obsEnc(current_obs_seqs[:, t, :])
